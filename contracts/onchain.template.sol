...
contract GippityOnchainNFT is ERC721A, DefaultOperatorFilterer, ReentrancyGuard, Ownable {
    ...
    constructor() ERC721A(unicode"{{COLLECTION_NAME}}", unicode"{{SYMBOL}}") {
        {{TIERS_ARRAYS}}
        ...
        randomSeedData = uint(keccak256(abi.encodePacked(tx.gasprice,block.number,block.timestamp,block.difficulty,blockhash(block.number - 1),msg.sender)));
    }

    // --- Rest of the contract code unchanged! ---
    // (Minting, metadata, trait upload, withdrawal, etc. — see your Indelible fork)
    ...
    function hashToSVG(
        ...
        svgBytes.appendSafe(
            abi.encodePacked(backgroundColor, ";background-image:url(")
        );
